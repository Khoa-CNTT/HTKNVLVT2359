...
const ManageSalaryType = () => {
  ...
  useEffect(() => {
    try {
      let trashDatadelete = JSON.parse(
        localStorage.getItem("TrashDelete_SalaryType")
      );
  ...
  let handleTransfromTrash = (id) => {
    let dataDelete = dataSalaryType.filter((value) => value.code == id);
    let trashDatadelete = JSON.parse(
      localStorage.getItem("TrashDelete_SalaryType")
    );
    if (trashDatadelete) {
